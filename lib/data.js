...
export async function getSimilarProperties(propertyOrId, cityOrLimit = 3, propertyType, limit = 3) {
  try {
    await connectDB();

    let id, city, pType, maxItems;

    if (typeof propertyOrId === 'object' && propertyOrId !== null) {
      id = propertyOrId._id;
      city = propertyOrId.address?.city;
      pType = propertyOrId.propertyType;
      maxItems = typeof cityOrLimit === 'number' ? cityOrLimit : 3;
    } else {
      id = propertyOrId;
      city = typeof cityOrLimit === 'string' ? cityOrLimit : undefined;
      pType = propertyType;
      maxItems = typeof limit === 'number' ? limit : 3;
    }
    ...
    const orConditions = [];
    if (pType) orConditions.push({ propertyType: pType });
    if (city) orConditions.push({ 'address.city': { $regex: new RegExp(city, 'i') } });

    if (orConditions.length > 0) {
      query.$or = orConditions;
    }

    const properties = await Property.find(query).limit(maxItems).lean();
    if (properties && properties.length > 0) {
      return properties.map(serializeDoc);
    }
  } catch (error) {
  ...
  const targetId = typeof propertyOrId === 'object' ? propertyOrId?._id?.toString() : propertyOrId?.toString();
  const maxItems = typeof propertyOrId === 'object' ? (typeof cityOrLimit === 'number' ? cityOrLimit : 3) : (typeof limit === 'number' ? limit : 3);

  return FALLBACK_PROPERTIES.filter((p) => p._id.toString() !== targetId).slice(0, maxItems);
}
...
export async function getAdminProperties(page = 1, search = '') {
  ...
  const filtered = FALLBACK_PROPERTIES.filter(
    (p) =>
      !search ||
      p.title?.toLowerCase().includes(searchLower) ||
      p.address?.city?.toLowerCase().includes(searchLower) ||
      p.address?.locality?.toLowerCase().includes(searchLower)
  );
